...
-- ── NOTAS_AJUSTE (notas crédito / débito) ──
-- La columna anticipos de cuentas_cobro NO se escribe a mano: la mantiene el
-- trigger de abajo a partir de estas notas, igual que valor_recibido desde pagos.
-- (Sin comillas invertidas: este bloque vive dentro de un template literal de JS.)
create table if not exists notas_ajuste (
  id                uuid primary key default gen_random_uuid(),
  cuenta_cobro_id   uuid not null references cuentas_cobro(id) on delete cascade,
  tipo              text not null check (tipo in ('credito','debito')),
  valor             numeric not null check (valor > 0),
  motivo            text not null,
  -- Numero SIIGO de la factura que corrige. REFERENCIA, no llave foranea: la
  -- factura puede ser anterior a la app (existe en SIIGO, no aqui).
  factura_origen    integer,
  origen_en_app     boolean not null default false,
  pct_admin_origen  numeric,
  iva_pct_origen    numeric,
  fecha             date not null default current_date,
  creado_en         timestamptz not null default now()
);
create index if not exists idx_notas_cc on notas_ajuste(cuenta_cobro_id);
create index if not exists idx_notas_origen on notas_ajuste(factura_origen);

create or replace function recalcular_notas_cuenta() returns trigger as $fn$
declare
...
begin
  select coalesce(sum(case when tipo = 'credito' then valor else -valor end), 0)
    into v_neto from notas_ajuste where cuenta_cobro_id = v_cc;
  update cuentas_cobro set anticipos = v_neto where id = v_cc;
  return null;
end;
$fn$ language plpgsql set search_path = public;

drop trigger if exists trg_notas_recalc on notas_ajuste;
create trigger trg_notas_recalc
  after insert or update or delete on notas_ajuste
  for each row execute function recalcular_notas_cuenta();
...
alter table notas_ajuste enable row level security;
do $do$
declare r record;
begin
...
export function generarBackupSQL(datos) {
  ...
  // Contraste contra los conteos que /api/export verificó CONTRA LA BASE
  // (`select count(*)`), no contra lo que llegó por la red. Sin esto, una lectura
  // truncada produciría un archivo que declara como total lo que ya venía corto:
  // un respaldo incompleto con aspecto de completo. Ver lib/db.js.
  const verificado = datos._verificado || null;
  if (verificado) {
    for (const { nombre } of TABLAS) {
      const traidas = (datos[nombre] || []).length;
      const esperadas = verificado[nombre];
      if (esperadas != null && traidas !== esperadas) {
        throw new Error(
          `Respaldo abortado: "${nombre}" trajo ${traidas} filas y en la base hay ${esperadas}. ` +
          `No se genera un respaldo incompleto.`
        );
      }
    }
  }
  ...
  const p = [
    ...
    verificado
      ? "-- Integridad: los conteos de abajo se verificaron contra select count(*) en la base."
      : "-- Integridad: NO VERIFICADA (respuesta sin conteos; revisa /api/export).",
    "-- ============================================================================",
    "-- CÓMO RESTAURAR:",
    ...
    "alter table notas_ajuste disable trigger trg_notas_recalc;",
    "",
  ];
  ...
  p.push("alter table notas_ajuste enable trigger trg_notas_recalc;", "alter table pagos enable trigger trg_pagos_recalc;", "commit;", "");
  return { sql: p.join("\n"), resumen };
}
