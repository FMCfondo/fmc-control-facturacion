...
/**
 * Período de las garantías que cobra una cuenta de cobro: el mes ANTERIOR al de
 * elaboración, porque se factura mes vencido. Una cuenta hecha en septiembre
 * cobra las garantías otorgadas en agosto.
 *
 * Hasta sep 2026 el correo aplicaba esta regla y el documento no: el PDF decía
 * "(Septiembre 2026)" mientras el correo hablaba de "garantías del mes de
 * agosto", y la mutual no sabía cuál creer. La regla vive aquí para que las tres
 * salidas (PDF, pantalla, correo) no puedan volver a divergir.
 *
 * @returns {string|null} "Agosto 2026", o null si la cuenta no tiene mes.
 */
export function periodoGarantias(cuenta) {
  const mes = Number(cuenta?.mes), anio = Number(cuenta?.anio);
  if (!mes || !anio) return null;
  let m = mes - 1, a = anio;
  if (m < 1) { m = 12; a -= 1; } // enero cobra diciembre del año anterior
  return `${MESES[m - 1]} ${a}`;
}

/** Concepto que aparece en la línea de la cuenta de cobro (PDF y pantalla). */
export function descripcionServicio(cuenta, mutual) {
  if (!mutual) return cuenta?.notas || "Cuenta de cobro";
  const p = periodoGarantias(cuenta);
  return `SERVICIO DE COBERTURA DE CRÉDITOS${p ? ` (${p})` : ""}`;
}
