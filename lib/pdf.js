...
import { calcularTotalesCuenta, descripcionServicio } from "./cuenta";
...
export function generarPDFCuenta({ cuenta, mutual, items, facturas, fondo, logoBase64 }) {
  ...
  if (anticipos !== 0) {
    ...
    // Se rotula por lo que significa para la mutual, no por el nombre contable.
    const etq = anticipos > 0 ? "DESCUENTO POR AJUSTE:" : "CARGO POR AJUSTE:";
    totRow(ty, etq, (anticipos > 0 ? "−" : "+") + fmtPesos(Math.abs(anticipos))); ty += 6;
  }
