...
import { norm, parseMoneda, digitoVerificacion, limpiarNombre, separarNombres, limpiarCorreo, validarTelefono, limpiarId } from "./utils.js";
...
export const COLMAP = {
  ...
  valor: { pos: 22, keys: ["valor de la comision", "valor comision", "comision con iva", "comision mas iva", "comision + iva", "comision e iva", "iva incluido"] },
};
...
// Cuenta cuántas celdas de una columna parecen dinero (> 0).
function contarMoneda(dataRows, col) {
  let n = 0;
  for (const r of dataRows) { const m = parseMoneda(r[col]); if (m.ok && m.valor > 0) n++; }
  return n;
}
// Elige la columna de comisión: entre las que coinciden por encabezado, la que tiene MÁS
// valores monetarios por fila. Evita columnas de TOTAL casi vacías (mismo encabezado repetido)
// y soporta variantes de cabecera entre plantillas. Respaldo: la posición por defecto.
function elegirColumnaValor(headerCols, dataRows) {
  const n = headerCols.map((c) => norm(c || ""));
  const cand = [];
  n.forEach((h, i) => { if (COLMAP.valor.keys.some((k) => h.includes(k))) cand.push(i); });
  if (!cand.length) return COLMAP.valor.pos;
  return cand.reduce((best, c) => (contarMoneda(dataRows, c) > contarMoneda(dataRows, best) ? c : best), cand[0]);
}
...
export function procesarFilas(rows) {
  ...
  // La columna de comisión varía entre plantillas y a veces hay una columna "total" con
  // el mismo encabezado: elegimos la candidata con más valores monetarios por fila.
  if (tieneHeader) map.valor = elegirColumnaValor(rows[hdrIdx], dataRows);
  const records = [], errs = [];
  dataRows.forEach((cols, i) => {
    const cell = (k) => { const v = cols[map[k]]; return v == null ? "" : String(v).trim(); };
    const id = limpiarId(cell("id")), nombre = cell("nombre"), dir = cell("dir"), ciudadDepto = cell("ciudadDepto"), email = cell("email");
