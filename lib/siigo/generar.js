...
import { NIT_FONDO, CODIGO_PRODUCTO, DESC_PRODUCTO, CUENTA_CXC_ASOCIADOS, CUENTA_CXC_MUTUALES, OBS, IVA } from "./constantes.js";
...
export function generarArchivosSiigo({ records, mutual, fechaISO, factIni, cdc }) {
  ...
  const fD = records.map((r, i) => {
    const vfp = round2(r.valorComision), base = round6(r.valorComision / (1 + IVA));
...
// Contenido en base64 (para archivar en la nube).
export function aBase64(file) {
  return XLSX.write(file.wb, { type: "base64", bookType: file.opts.bookType || "xlsx" });
}
